...
export const LO_020103_P01: React.FC = () => {
  const {} = useModal();
  ...
  // 탭 아이템 정의
  const tabOptions = [
    { id: 'response', label: 'Request' },
    { id: 'system-prompt', label: 'Response' },
    // { id: 'assistant', label: 'System prompt' },
  ];
        ...
        <UIAlarmGroup>
          <section className='section-popup-content' style={{ padding: '0' }}>
            <UIPopupBody>
              ...
              {/* [251125_퍼블수정] 영역 삭제 수정 */}
              {/* <UIArticle className='!mt-[24px]'>
                <UIGroup gap={8} direction={'column'}>
                  <UITextArea2
                    value={'신한은행 모바일 앱에서 계좌 개설하려면 어떻게 해야 하나요?'}
                    style={{ pointerEvents: 'none' }}
                    onChange={_e => {}}
                    placeholder='내용이 표시됩니다...'
                  />
                </UIGroup>
              </UIArticle> */}
              <UIArticle>
                <UIUnitGroup gap={16} direction='column' align='center'>
                  <div className='flex justify-between w-full'>
                    <UITypography variant='title-4' className='secondary-neutral-900 text-sb'>
                      {activeTab === 'system-prompt' ? 'Dictionary' : 'JSON'}
                    </UITypography>
