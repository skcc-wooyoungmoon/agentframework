...
import { UIArticle, UIFormField, UIGroup, UIInput, UIList, UITextArea2, UIPopupBody, UIPopupFooter, UIPopupHeader, UIUnitGroup } from '@/components/UI/molecules';
import { UILayerPopup, UIPopupAside } from '@/components/UI/organisms';

import { DesignLayout } from '../../components/DesignLayout';

export const PR_040101_P01: React.FC = () => {
            ...
            <UIArticle>
              <UIGroup gap={16} direction='column'>
                ...
                <div>
                  {/* 파일 목록 */}
                  {/* [251114_퍼블수정] 파일 업로드 영역 수정 */}
                  {files.length > 0 && <UIFileBox variant='default' size='full' items={files.map(fileName => ({ fileName, fileSize: 99 }))} onFileRemove={handleFileRemove} className='w-full' />}
                </div>
                <div>
                  <UIList
                    gap={4}
