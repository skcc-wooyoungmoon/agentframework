...
export type AgentGraph = {
  ...
  graph: {
    ...
    nodes: Array<{
      id: string;
      type: string;
      position: { x: number; y: number }; // position은 필수
      width?: number; // 노드 너비
      height?: number; // 노드 높이
      data: any;
      style?: any;
...
export interface RetrievalOptions {
  doc_format_metafields: string[] | null;
...
export interface EnsembleGatherCnf {
  repo_ids: string[];
  weights: number[];
}

export interface RetrievalStepContextRefiner {
  llm_chain: RetrieverLlmChin | null;
  gather_cnf: EnsembleGatherCnf | null;
}
...
export type CustomNode = Node & {
  ...
  data: { innerData: CustomNodeInnerData } & (
    ...
    | GeneratorDataSchema
    | AgentAppDataSchema // 20250913 추가
    | CoderDataSchema
    | ReACTorDataSchema
    | CategorizerDataSchema
    | ConditionDataSchema
    | UnionDataSchema
    | ReviewerDataSchema // 20250913 추가
    | ToolDataSchema
    | ReRankerDataSchema
...
export type GeneratorDataSchema = {
  ...
  tools: Tool[];
  mcp_selections?: any[] | null; // MCP 선택 정보 추가
  input_keys: InputKeyItem[];
  output_keys: OutputKeyItem[];
};
