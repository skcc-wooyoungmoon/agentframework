...
import { getPhoenixProjectId, useGetAgentGraphCode } from '@/services/agent/builder2/agentBuilder.services';
import { type PhoenixProjectResponse } from '@/services/agent/builder2/types';
...
export const GraphController = ({ onLayoutClick, onDeployClick, onSaveClick: _onSaveClick, onChatClick, readOnly = false, unsavedChanges = false }: GraphControllerProps) => {
  const [agent] = useAtom(agentAtom);
  const { openAlert, openModal } = useModal();
  const [exportCode, setExportCode] = useState<string>('');
  // Agent Graph Code 조회 훅
  const { refetch: refetchGraphCode } = useGetAgentGraphCode(
    { graphId: agent?.id || '', credentialType: 'token' },
    { enabled: false } // 버튼 클릭 시에만 조회
  );
  ...
  const handleExportButton = async () => {
    ...
    try {
      const result = await refetchGraphCode();

      let code: string | null = result.data?.data;
      if (code) {
        setExportCode(code);
  ...
  return (
    <div
      className={'flex flex-row justify-end gap-2 py-4'}
    ...
    >
      <div className='flex items-center gap-2' style={{ opacity: readOnly ? 0.6 : 1 }}>
        <button
          type='button'
          className='!rounded-md !w-8 h-8 !px-0 flex items-center justify-center bg-white border border-[#DCE2ED] hover:bg-gray-50'
          style={{
          ...
          onClick={() => {
            openModal({
              title: 'Key Tables',
              type: 'medium',
              body: <KeyTables disabledKeyIn={true} />,
              showFooter: true,
              cancelText: '닫기',
              confirmText: '',
            });
        ...
        <button
          type='button'
          className='h-8 p-0 w-[50px] text-sm font-normal bg-white border border-[#DCE2ED] ag-btn-hover cursor-pointer flex items-center justify-center rounded-[6px] text-[#242A34]'
          style={{
            pointerEvents: readOnly ? 'none' : 'auto',
            zIndex: 1002,
            cursor: readOnly ? 'not-allowed' : 'pointer',
