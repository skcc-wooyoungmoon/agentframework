...
import { messagesAtom, regenerateAtom, regenerateTargetIndexAtom, setHumanRegenAtom } from '@/components/builder/atoms/messagesAtom.ts';
...
const MessageIn = ({ text, time, elapsedTime, index, isRegenerating }: ChatMessageInProps) => {
  ...
  const [, setRegenerateTargetIndex] = useAtom(regenerateTargetIndexAtom);
  const [, setHumanRegen] = useAtom(setHumanRegenAtom);

  const handleRegenerate = () => {
    // 현재 AI 메시지의 인덱스 저장
    setRegenerateTargetIndex(index);

    // 해당 human 메시지 찾아서 regen: true로 설정
    for (let i = index - 1; i >= 0; i--) {
      if (messages[i].type === ChatType.HUMAN) {
        setHumanRegen(i); // human 메시지의 regen을 true로 설정
        setRegenerate(messages[i].content);
        break;
      }
    }
  };
