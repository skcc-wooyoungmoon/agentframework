package com.skax.aiplatform.client.sktai.model.dto.request;

import java.time.LocalDateTime;
import java.util.List;

import com.fasterxml.jackson.annotation.JsonProperty;

import io.swagger.v3.oas.annotations.media.Schema;
import lombok.AllArgsConstructor;
import lombok.Builder;
import lombok.Data;
import lombok.NoArgsConstructor;

/**
 * SKTAI Model Import 요청 DTO
 * 
 * <p>SKTAI Model 시스템에서 Model Import를 위한 요청 데이터 구조입니다.
 * ModelExportResponse와 동일한 구조를 가집니다.</p>
 *
 * @since 2025-11-11
 * @version 1.0
 */
@Data
@NoArgsConstructor
@AllArgsConstructor
@Builder
@Schema(description = "SKTAI Model Import 요청")
public class ModelImportRequest {
    
    @JsonProperty("model")
    @Schema(description = "Import용 Model 데이터 (Export Response와 동일한 구조)")
    private ImportModel model;
    
    @JsonProperty("provider")
    @Schema(description = "모델 제공자 정보")
    private ProviderInfo provider;
    
    @JsonProperty("languages")
    @Schema(description = "지원 언어 목록")
    private List<LanguageInfo> languages;
    
    @JsonProperty("tasks")
    @Schema(description = "수행 가능한 작업 목록")
    private List<TaskInfo> tasks;
    
    @JsonProperty("tags")
    @Schema(description = "모델 태그 목록")
    private List<TagInfo> tags;
    
    @JsonProperty("endpoints")
    @Schema(description = "엔드포인트 목록")
    private List<Object> endpoints;
    
    @JsonProperty("custom_runtimes")
    @Schema(description = "커스텀 런타임 목록")
    private List<CustomRuntimeInfo> customRuntimes;

    // @JsonProperty("policy")
    // @Schema(description = "정책 목록")
    // private List<PolicyRequest> policy;
    
    /**
     * Import용 Model 데이터
     */
    @Data
    @NoArgsConstructor
    @AllArgsConstructor
    @Builder
    @Schema(description = "Import용 Model 데이터")
    public static class ImportModel {
        
        @JsonProperty("id")
        @Schema(description = "모델 ID", format = "uuid")
        private String id;
        
        @JsonProperty("display_name")
        @Schema(description = "모델 표시 이름")
        private String displayName;
        
        @JsonProperty("name")
        @Schema(description = "모델 관리 이름")
        private String name;
        
        @JsonProperty("type")
        @Schema(description = "모델 타입", example = "language")
        private String type;
        
        @JsonProperty("description")
        @Schema(description = "모델 설명")
        private String description;
        
        @JsonProperty("size")
        @Schema(description = "모델 파라미터 크기")
        private String size;
        
        @JsonProperty("token_size")
        @Schema(description = "토큰 크기")
        private String tokenSize;
        
        @JsonProperty("inference_param")
        @Schema(description = "추론 파라미터")
        private Object inferenceParam;
        
        @JsonProperty("quantization")
        @Schema(description = "양자화 정보")
        private Object quantization;
        
        @JsonProperty("dtype")
        @Schema(description = "데이터 타입")
        private String dtype;
        
        @JsonProperty("serving_type")
        @Schema(description = "서빙 타입", example = "serverless")
        private String servingType;
        
        @JsonProperty("is_private")
        @Schema(description = "프라이빗 모델 여부")
        private Boolean isPrivate;
        
        @JsonProperty("is_valid")
        @Schema(description = "유효성 여부")
        private Boolean isValid;
        
        @JsonProperty("license")
        @Schema(description = "라이선스 정보")
        private String license;
        
        @JsonProperty("readme")
        @Schema(description = "README 문서")
        private String readme;
        
        @JsonProperty("path")
        @Schema(description = "모델 파일 경로")
        private String path;
        
        @JsonProperty("provider_id")
        @Schema(description = "모델 제공자 ID", format = "uuid")
        private String providerId;
        
        @JsonProperty("project_id")
        @Schema(description = "프로젝트 ID", format = "uuid")
        private String projectId;
        
        @JsonProperty("default_params")
        @Schema(description = "기본 파라미터")
        private Object defaultParams;
        
        @JsonProperty("last_version")
        @Schema(description = "마지막 버전 번호")
        private Integer lastVersion;
        
        @JsonProperty("is_custom")
        @Schema(description = "커스텀 모델 여부")
        private Boolean isCustom;
        
        @JsonProperty("custom_code_path")
        @Schema(description = "커스텀 코드 파일 경로")
        private String customCodePath;
        
        @JsonProperty("training_id")
        @Schema(description = "트레이닝 ID", format = "uuid")
        private String trainingId;
        
        @JsonProperty("created_at")
        @Schema(description = "생성 일시", format = "date-time")
        private LocalDateTime createdAt;
        
        @JsonProperty("updated_at")
        @Schema(description = "수정 일시", format = "date-time")
        private LocalDateTime updatedAt;
    }
    
    /**
     * Provider 정보
     */
    @Data
    @NoArgsConstructor
    @AllArgsConstructor
    @Builder
    @Schema(description = "Provider 정보")
    public static class ProviderInfo {
        
        @JsonProperty("name")
        @Schema(description = "Provider 이름", maxLength = 255)
        private String name;
        
        @JsonProperty("description")
        @Schema(description = "Provider 설명")
        private String description;
        
        @JsonProperty("logo")
        @Schema(description = "로고 URL")
        private String logo;
        
        @JsonProperty("id")
        @Schema(description = "Provider ID", format = "uuid")
        private String id;
        
        @JsonProperty("created_at")
        @Schema(description = "생성 일시", format = "date-time")
        private LocalDateTime createdAt;
        
        @JsonProperty("updated_at")
        @Schema(description = "수정 일시", format = "date-time")
        private LocalDateTime updatedAt;
    }
    
    /**
     * Language 정보
     */
    @Data
    @NoArgsConstructor
    @AllArgsConstructor
    @Builder
    @Schema(description = "Language 정보")
    public static class LanguageInfo {
        
        @JsonProperty("name")
        @Schema(description = "언어 이름", maxLength = 64)
        private String name;
        
        @JsonProperty("id")
        @Schema(description = "언어 ID")
        private Integer id;
        
        @JsonProperty("created_at")
        @Schema(description = "생성 일시", format = "date-time")
        private LocalDateTime createdAt;
        
        @JsonProperty("updated_at")
        @Schema(description = "수정 일시", format = "date-time")
        private LocalDateTime updatedAt;
    }
    
    /**
     * Task 정보
     */
    @Data
    @NoArgsConstructor
    @AllArgsConstructor
    @Builder
    @Schema(description = "Task 정보")
    public static class TaskInfo {
        
        @JsonProperty("name")
        @Schema(description = "작업 이름", maxLength = 64)
        private String name;
        
        @JsonProperty("id")
        @Schema(description = "작업 ID")
        private Integer id;
        
        @JsonProperty("created_at")
        @Schema(description = "생성 일시", format = "date-time")
        private LocalDateTime createdAt;
        
        @JsonProperty("updated_at")
        @Schema(description = "수정 일시", format = "date-time")
        private LocalDateTime updatedAt;
    }
    
    /**
     * Tag 정보
     */
    @Data
    @NoArgsConstructor
    @AllArgsConstructor
    @Builder
    @Schema(description = "Tag 정보")
    public static class TagInfo {
        
        @JsonProperty("name")
        @Schema(description = "태그 이름")
        private String name;
        
        @JsonProperty("id")
        @Schema(description = "태그 ID")
        private Integer id;
        
        @JsonProperty("created_at")
        @Schema(description = "생성 일시", format = "date-time")
        private LocalDateTime createdAt;
        
        @JsonProperty("updated_at")
        @Schema(description = "수정 일시", format = "date-time")
        private LocalDateTime updatedAt;
    }
    
    /**
     * Custom Runtime 정보
     */
    @Data
    @NoArgsConstructor
    @AllArgsConstructor
    @Builder
    @Schema(description = "Custom Runtime 정보")
    public static class CustomRuntimeInfo {
        
        @JsonProperty("model_id")
        @Schema(description = "모델 ID", format = "uuid")
        private String modelId;
        
        @JsonProperty("image_url")
        @Schema(description = "커스텀 Docker 이미지 URL", maxLength = 1000)
        private String imageUrl;
        
        @JsonProperty("use_bash")
        @Schema(description = "bash 사용 여부", example = "false")
        private Boolean useBash;
        
        @JsonProperty("command")
        @Schema(description = "컨테이너 실행 명령어 배열")
        private List<String> command;
        
        @JsonProperty("args")
        @Schema(description = "컨테이너 실행 인자 배열")
        private List<String> args;
        
        @JsonProperty("id")
        @Schema(description = "Custom Runtime ID", format = "uuid")
        private String id;
        
        @JsonProperty("created_at")
        @Schema(description = "생성 일시", format = "date-time")
        private LocalDateTime createdAt;
        
        @JsonProperty("updated_at")
        @Schema(description = "수정 일시", format = "date-time")
        private LocalDateTime updatedAt;
    }
}

