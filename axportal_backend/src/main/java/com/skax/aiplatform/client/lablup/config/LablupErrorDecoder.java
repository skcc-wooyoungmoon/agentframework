...
/**
 * Lablup API 전용 에러 디코더
 * 
 * <p>Lablup API 호출 시 발생하는 HTTP 에러를 애플리케이션 예외로 변환합니다.
 ...
 * Lablup FeignClient 설정에서만 사용되어 다른 FeignClient에 영향을 주지 않습니다.</p>
 * 
 * @author ByounggwanLee
 * @since 2025-10-02
 * @version 2.0 - 상세한 에러 메시지 파싱 추가, 422 오류 특별 처리
 * @updated 2025-10-17 - JSON 응답 파싱 및 Database error 특별 처리 추가
 */
@Slf4j
public class LablupErrorDecoder implements ErrorDecoder {
        ...
        switch (status) {
            ...
            case 429:
                return new BusinessException(ErrorCode.EXTERNAL_SERVICE_ERROR, 
                    "Lablup API 호출 한도 초과" + (errorDetail != null ? ": " + errorDetail : ""));
            case 500:
            case 502:
            case 503:
            case 504:
                return new BusinessException(ErrorCode.EXTERNAL_API_SERVER_ERROR, 
                    "Lablup API 서버 내부 오류" + (errorDetail != null ? ": " + errorDetail : ""));
            default:
                return new BusinessException(ErrorCode.EXTERNAL_SERVICE_ERROR, 
                    String.format("Lablup API 호출 실패 (Status: %d)", status) + 
    ...
    private String extractErrorDetail(Response response) {
        ...
        try {
            ...
            log.warn("Lablup API 응답 바디 파싱 실패: {}", e.getMessage());
            return null;
        } catch (Exception e) {
            log.warn("Lablup API 예상치 못한 오류로 응답 바디 파싱 실패: {}", e.getMessage());
            return null;
        }
    }
}