...
/**
 * IONE Open API Key 등록 응답 DTO
 * 
 * <p>IONE API Gateway에서 새로운 Open API Key를 등록하는 응답 데이터 구조입니다.
 * 실제 IONE API Swagger 문서의 RegApiKeyResult 스키마를 기반으로 구현되었습니다.</p>
 * 
...
@Builder
@Schema(
    description = "IONE Open API Key 등록 응답",
    example = """
        {
          "result": {
            "success": true,
...
public class RegApiKeyResult {
    ...
    /**
     * 등록된 Open API Key 문자열
     */
    @JsonProperty("data")
    @Schema(
        description = "등록된 Open API Key 문자열",
     ...
     * 등록된 Open API Key 조회
     * 
     * @return 등록된 Open API Key (실패 시 null)
     */
    public String getRegisteredApiKey() {
