...
/**
 * SKTAI RAG Evaluation 결과 생성 요청 DTO
 * 
 * <p>SKTAI Evaluation 시스템에서 RAG Evaluation 결과를 생성하기 위한 요청 데이터 구조입니다.
 * RAG 모델 정보, 평가 정보, 작업 결과, 메트릭 등을 포함합니다.</p>
 * 
 * <h3>필수 정보:</h3>
 * <ul>
 *   <li><strong>rag_evaluation_id</strong>: RAG 평가 식별자</li>
 ...
 * <h3>사용 예시:</h3>
 * <pre>
 * RagEvaluationResultCreateRequest request = RagEvaluationResultCreateRequest.builder()
 *     .ragEvaluationId(1)
...
@Builder
@Schema(
    description = "SKTAI RAG Evaluation 결과 생성 요청 정보",
    example = """
        {
          "id": null,
...
public class RagEvaluationResultCreateRequest {
     ...
     * 모델 식별자
     * 
     * <p>RAG 평가를 수행한 모델의 UUID입니다.
     * Fine-tuned 모델이 아닌 경우 null로 설정할 수 있습니다.</p>
     */
    @JsonProperty("model_id")
    @Schema(
        description = "RAG 평가를 수행한 모델의 UUID", 
     ...
     * RAG 평가 식별자
     * 
     * <p>결과가 속한 RAG 평가의 ID입니다.</p>
     */
    @JsonProperty("rag_evaluation_id")
    @Schema(
        description = "RAG 평가 ID", 
        example = "1",
        required = true
    )
     ...
     * 작업명
     * 
     * <p>수행된 구체적인 RAG 평가 작업의 이름입니다.</p>
     */
    @JsonProperty("task")
    @Schema(
        description = "수행된 RAG 평가 작업명", 
        example = "retrieval_accuracy",
        required = true
    )
    private String task;
