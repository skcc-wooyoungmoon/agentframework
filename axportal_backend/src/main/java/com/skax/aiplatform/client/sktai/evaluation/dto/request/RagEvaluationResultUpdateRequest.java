...
/**
 * SKTAI RAG Evaluation 결과 업데이트 요청 DTO
 * 
 * <p>SKTAI Evaluation 시스템에서 백그라운드에서 RAG Evaluation 결과를 업데이트하기 위한 요청 데이터 구조입니다.
 * 결과 파일을 읽어서 데이터베이스에 업데이트하는 작업을 수행합니다.</p>
 * 
 * <h3>업데이트 프로세스:</h3>
 * <ul>
 *   <li>RAG Evaluation 로그 ID를 기준으로 결과 파일을 읽습니다</li>
 ...
 * <h3>사용 예시:</h3>
 * <pre>
 * RagEvaluationResultUpdateRequest request = RagEvaluationResultUpdateRequest.builder()
 *     .ragEvaluationLogId(123)
...
@Builder
@Schema(
    description = "SKTAI RAG Evaluation 결과 업데이트 요청 정보",
    example = """
        {
          "rag_evaluation_log_id": 123,
          "policy": null
        }
        """
)
public class RagEvaluationResultUpdateRequest {
    ...
    /**
     * RAG Evaluation 로그 식별자
     * 
     * <p>업데이트할 결과와 연관된 RAG Evaluation 로그의 ID입니다.
     * 이 ID를 기준으로 해당 로그의 결과 파일을 찾아 업데이트를 수행합니다.</p>
     */
    @JsonProperty("rag_evaluation_log_id")
    @Schema(
        description = "업데이트할 RAG Evaluation 로그 ID", 
        example = "123",
        required = true
    )
